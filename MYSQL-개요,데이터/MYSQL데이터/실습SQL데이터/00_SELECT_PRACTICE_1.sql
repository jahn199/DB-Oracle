...
MYSQL 회사 SQL을 이용해서 관리를 하고, 각 데이터베이스에 유저가 접근하기 위해서는 
1. 유저 아이디, 비밀번호로 접속
2. DEUM 데이터베이스에 접속해서 회원정보나 게시글 관리 테이블 확인하길 원한다면 
   USE DEUM;
...
CREATE TABLE USER (
    user_id INT PRIMARY KEY auto_increment,        -- 유저 가입순서
...
-- USERPROFILE 테이블 생성alter

-- USER_ID 기본키 자동증가 
-- USERNAME 문자열 빈 값 못 들어가게 설정(50 BYTE) 
-- EMAIL 문자열 빈값 못 들어가게 설정 (100 BYTE)
-- BIRTHDATE DATE
...
-- khtdb에 USERPROFILE 이라는 테이블이 존재하지 않으므로 CREATE TABLE USERPROFILE을 이용해서
-- USERPROFILE 테이블을 생성하거나, 테이블 오타가 있는지 확인! 
CREATE TABLE USERPROFILE (
  USER_ID INT PRIMARY KEY AUTO_INCREMENT,        -- 기본키, 자동으로 가입할 때 순차적으로 번호 증가 기입
  USERNAME VARCHAR(50) NOT NULL,                 -- 이름(빈칸 허용 불가)
  EMAIL VARCHAR(100) NOT NULL,                   -- 이메일(빈칸 허용 불가)
...
VALUES
    ...
    -- 테이블 삭제 * 주의 테이블 삭제 함부로 하지 말것; 반드시 혼자하더라도 상의 후 삭제할 것 
    -- 테이블 삭제 = DROP TABLE; 테이블 내 데이터 삭제 
    -- DELETE FROM 테이블명 WHERE = 테이블에서 하나의 행 삭제할 조건; 
